...
3. If Camera is "subjective" or "subjective_portree": this is NON-NEGOTIABLE — every single generation MUST be true first-person POV. IMPORTANT — identify the physical context before describing camera and arm geometry. Two sub-cases: SUB-CASE A — Confined or enclosed space (under a sink, inside a cabinet, under a car, behind a panel, inside a narrow opening): Camera axis is nearly HORIZONTAL, pointing slightly UPWARD into the enclosed space — the viewer is OUTSIDE leaning forward to peer under a low ceiling or into a tight cavity. Both forearms enter the frame from the LEFT EDGE and RIGHT EDGE at MID-FRAME height, arms nearly horizontal, converging toward the center where the work is. The work (pipes, mechanism, components) sits at the CONVERGENCE POINT in the CENTER or upper-center of the frame. The enclosed space interior is visible above and around the work. SUB-CASE B — Open surface work (floor, workbench, countertop, ground, outdoor flat surface): Camera axis points DOWNWARD at 45 degrees toward the surface. Both forearms enter from the BOTTOM-LEFT and BOTTOM-RIGHT edges, converging toward the lower center of the frame. The LLM must select SUB-CASE A or B based on the scene before writing any camera or arm geometry. Under-sink plumbing = always SUB-CASE A. Any side-view, observer-view, or single-hand framing is a CRITICAL FAILURE of this rule. CRITICAL for subjective POV action description: never describe both hands holding a single tool together (no "gripping pliers with both hands", no "both hands on a wrench"). Instead, always decompose the hand action into two distinct complementary roles: LEFT HAND: [stabilizing / holding / supporting one component] RIGHT HAND: [tightening / adjusting / turning the other component] For plumbing under a sink: left hand braces the P-trap body, right hand turns the slip nut or coupling. For carpentry: left hand positions the piece, right hand drives the tool. This asymmetric dual-role is what creates natural symmetric arm spread from both sides of the frame. SHARPNESS RULE: both forearms and hands must be in sharp focus — no motion blur, no depth-of-field blur on the arms. The arms are the closest elements to the viewer and must be razor sharp. Blur is only acceptable on the far background beyond the work object.
